SELECT
    ...
    (m.modal_price - LAG(m.modal_price) OVER (PARTITION BY m.commodity, m.district ORDER BY m.date)) / NULLIF(LAG(m.modal_price) OVER (PARTITION BY m.commodity, m.district ORDER BY m.date), 0) * 100 AS price_change_pct
